...
public class RecetaResource {
    ...
    @RequestMapping(value = "/recetas",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<Receta> updateReceta(@RequestBody Receta receta) throws URISyntaxException {
   ...
   /* @RequestMapping(value = "/recetasComentarios/{id}",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Comentario>> getRecetaComentarios(@PathVariable long id, Pageable pageable)
        throws URISyntaxException {
        log.debug("REST request to get a page of Recetas");
        Page<Comentario> page = recetaRepository.findRecetasComentarios(id, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/recetas/findRecetasComentarios");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
